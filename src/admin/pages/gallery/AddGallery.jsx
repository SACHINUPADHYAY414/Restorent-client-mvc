...
const AddEditGallery = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title.trim() || !description.trim() || !type.trim() || (!image && !isEditMode)) {
      customToast({
        severity: ERROR,
        summary: OPPS_MSG,
    ...
    try {
      ...
      formPayload.append("type", type.trim());
      if (image) {
        formPayload.append("image", image);
      }

      if (isEditMode) {
        await api.put(`/gallery/update/${id}`, formPayload, {
        ...
        await api.post(`/gallery/add`, formPayload, {
          headers: { "Content-Type": "multipart/form-data" }
        });
        customToast({
          severity: SUCCESS,
          summary: "Success",
          detail: "Gallery item added successfully!",
          life: 3000
        });
      }
  ...
  return (
    <div className="w-100" style={{ minHeight: "61vh" }}>
      <h4 className="page-h">{isEditMode ? "Edit Gallery" : "Add Gallery"}</h4>
      <div className="card">
        <div className="card-body">
          <Form onSubmit={handleSubmit}>
            ...
            <Button type="submit" variant="primary">
              {isEditMode ? "Update Gallery" : "Submit"}
            </Button>
          </Form>
        </div>
      </div>
